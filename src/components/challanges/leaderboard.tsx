...
export default function Leaderboard() {
  ...
  const { data: tmsleaderboardData, refetch: tmsleaderboardDataFatching } =
    useQuery({
          ...
          <div className="rounded-b-lg overflow-hidden">
            ...
            <div className="max-h-[500px] overflow-y-auto flex flex-col gap-4 relative">
              {tmsleaderboardData?.data.map((entry: any, i: number) => (
                <div
                  key={entry._id}
                  className={`grid rounded-lg grid-cols-4 items-center text-center py-3 ${getRowColor(i, false)}`}
                >
        ...
        <div className="rounded-b-lg px-3 overflow-hidden">
          ...
          <div className="max-h-[500px] pr-3 flex flex-col gap-4 overflow-y-auto relative">
            {totalLeaderboard?.data.map((entry, i) => (
              <div
                key={entry._id}
                className={`grid grid-cols-4 border items-center rounded-lg text-center py-3 ${getRowColor(i, false)}`}
              >
                <div className="font-bold text-white">{i + 1}</div>
                <div className="flex items-center text-left">
                  <div>
